...
function Effects() {
  ...
  const hue = useControl("Hue", { group: "postproc", value: 3.11, min: 0, max: Math.PI * 2, type: "number" })
  const saturation = useControl("saturation", { group: "postproc", value: 2.05, min: 0, max: Math.PI * 2, type: "number" })
  const noise = useControl("noise", { group: "postproc", value: 0.47, min: 0, max: 1, type: "number" })
